...
1. **Client (User) Sends Request**  
    → The user sends a request (e.g., [`http://myapp.com`](http://myapp.com) or a public IP) over the internet.
    
2. **DNS Resolution** *(optional but common)*  
    → DNS translates the domain name to the public IP address (usually of the Load Balancer).
    
3. **Internet Gateway (IGW)**  
    → The request first enters your AWS VPC through the **Internet Gateway**.  
    💡 *It’s the entry/exit point between the internet and your VPC.*
    
4. **Route Table**  
    → Inside the VPC, the **Route Table** decides where the traffic should go next  
    (e.g., to the subnet where the Load Balancer resides).
    
5. **Network ACL (NACL)**  
    → Acts as a **stateless firewall** at the **subnet level**.  
    → It checks inbound/outbound rules — if allowed, traffic continues.
    
6. **Load Balancer (if used)**  
    → The **Load Balancer** receives the request, checks its **Security Group** rules,  
    and then forwards the request to the EC2 instance(s) in its **Target Group**.
    
7. **Security Group (of EC2)**  
    → The **EC2 Security Group** (stateful firewall) checks if the request is allowed (e.g., port 80/443).  
    → If yes, the traffic reaches the instance.
    
8. **EC2 Instance**  
    → The request hits your web server (e.g., Nginx, Apache, Node.js) running on the EC2 instance.  
    → The server processes the request and sends the response back through the same path (in reverse).
